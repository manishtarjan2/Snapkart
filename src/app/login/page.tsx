...
import { EyeIcon, EyeOff, Key, Leaf, Loader2, Lock, LogIn, Mail } from 'lucide-react'
  ...
  const handleLogin = async (e: FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // page.tsx handles role-based redirects (superAdmin → /super-admin, etc.)
        router.push("/")
      }
    } catch (err) {
      console.log(err)
  ...
  return (
    <div className='flex flex-col items-center justify-center min-h-screen text-center p-6'>
      ...
      >
        ...
        {error && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            className="bg-red-50 border border-red-200 rounded-xl px-4 py-3 text-red-600 text-sm text-left"
          >
            {error}
          </motion.div>
        )}
      ...
      <div className="mt-8 pt-6 border-t border-gray-100 w-full max-w-sm text-center">
        <p className="text-xs text-gray-400 mb-3">Are you a staff member or admin?</p>
        <button
          onClick={() => router.push("/admin-login")}
          className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-gray-700 bg-gray-900 text-white text-sm font-medium hover:bg-gray-800 transition-all shadow-sm"
        >
